...
MULTITASK_CONFIGS = {
    ...
    "FALL_DETECT_FALL_CLASSIFY": {"fall": 2, "fall_classify": 4},
...
def sensor_sets(chest, left, right):
    # Early fusion combinations (CHEST_LEFT, CHEST_LEFT_RIGHT, etc.) are
    # disabled to avoid OOM on memory-constrained machines: concatenating
    # 3 sensors at float32 requires >3 GB peak during normalization.
    # Multi-sensor comparison is covered by Late Fusion ensembles (ENSEMBLE_*)
    # which train independent models and average probabilities at inference.
    return {
        "CHEST": chest,
        "LEFT": left,
        "RIGHT": right,
...
def prepare_data(X_train, model_type, save_dir, X_test=None, prefix=""):
    ...
    # Normalize in-place to avoid allocating extra temporary arrays.
    # X[train_idx] and X[test_idx] are already copies (fancy indexing),
    # so modifying them in-place is safe.
    ...
    if model_type in CONV_MODELS:
        ...
        X_test = None if X_test is None else np.ascontiguousarray(X_test.transpose(0, 2, 1))
    elif model_type == "MLP":
