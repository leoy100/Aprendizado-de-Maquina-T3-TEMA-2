...
MULTITASK_MODES_TO_RUN = [
    # Head 1: queda/não-queda (binary)
    # Head 2: tipo/orientação final da queda (4 classes) e 1 para não-quedas
    "FALL_DETECT_FALL_CLASSIFY",
]
...
def print_experiment_summary():
    ...
    if "MULTI_TASK_MATRIX" in EXPERIMENTS_TO_RUN:
        print(f"Multi-task modes: {MULTITASK_MODES_TO_RUN}")
        print(f"Multi-task model: {MULTI_TASK_MODEL}")
        print(f"Multi-task loss weights: {MULTI_TASK_WEIGHTS}")
