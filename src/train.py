...
if __name__ == "__main__":
    ...
    for experiment in config.EXPERIMENTS_TO_RUN:
        ...
        elif experiment == "MULTI_TASK_MATRIX":
            print(f"\n{'=' * 72}")
            print("MULTI-TASK MODEL MATRIX")
            print(f"MODES: {config.MULTITASK_MODES_TO_RUN}")
            print(f"BASE MODEL: {config.MULTI_TASK_MODEL}")
            print(f"LOSS WEIGHTS: {config.MULTI_TASK_WEIGHTS}")
            print(f"{'=' * 72}")

            for mode in config.MULTITASK_MODES_TO_RUN:
                run_multitask(
                    mode=mode,
